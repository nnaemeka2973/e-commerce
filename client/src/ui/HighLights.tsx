...
import type { HighlightsType } from "../../type";
  ...
  return (
    <Container className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {highlightsData.map((item: HighlightsType) => (
        <div
          key={item?._id}
          className="relative h-60 rounded-lg shadow-md 
          ...
          >
            ...
            <Link to={item?._base} className="text-base font-normal ">
             <p className=" duration-200 
                relative inline-block w-fit overflow-hidden group"> {item?.buttonTitle} 
                
                 <span className="absolute h-px w-full bg-white 
               bottom-0.5 left-0 transform -translate-x-[105%]
